    ...
    def __assignToArmature(self, armObj, action_name=None):
        ...
        for name, keyFrames in boneAnim.items():
            ...
            if num_frame < 1:
                continue
            if DEBUG_MODE:
                target_bones = ["左足ＩＫ", "左足D", "左ひざD", "左足首D"]
                if name not in target_bones:
                    continue
            ...
            for k, x, y, z, r0, r1, r2, r3 in zip(keyFrames, *fcurves, strict=False):
                ...
                if DEBUG_MODE:
                    # 輸出 VMD 原始數據
                    print(f"骨骼: {name}, 幀: {k.frame_number}")
                    print(
                        f"  原始位置: X={k.location[0]:.3f}, Y={k.location[1]:.3f}, Z={k.location[2]:.3f}"
                    )
                    print(
                        f"  原始旋轉: X={k.rotation[0]:.3f}, Y={k.rotation[1]:.3f}, Z={k.rotation[2]:.3f}, W={k.rotation[3]:.3f}"
                    )
                    print(f"  轉換後位置: X={loc[0]:.3f}, Y={loc[1]:.3f}, Z={loc[2]:.3f}")
                    if hasattr(curr_rot, "__len__") and len(curr_rot) >= 4:
                        print(
                            f"  轉換後旋轉: X={curr_rot[0]:.3f}, Y={curr_rot[1]:.3f}, Z={curr_rot[2]:.3f}, W={curr_rot[3]:.3f}"
                        )
                    else:
                        print(f"  轉換後旋轉: {curr_rot}")
                    print("---")
