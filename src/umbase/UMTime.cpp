...
namespace umbase
{
	...
	UMTime::UMTime(const std::string& message)
		: message_(message), show_message_box_(false)
	{
	...
	UMTime::UMTime(const std::string& message, bool show_message_box)
		: message_(message), show_message_box_(show_message_box)
	{
		initial_time_ = current_time();
	}
	...
	UMTime::~UMTime()
	{
		...
		std::string message(
			message_ + ": " + UMStringUtil::number_to_string(seconds) + "s " + UMStringUtil::number_to_string(mills) + "ms");
