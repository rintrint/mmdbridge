...
// 輔助函數：根據骨骼索引計算 VMD bone frame
static vmd::VmdBoneFrame calculate_bone_frame(
    int model_index,                    // 模型索引 (i)
    int bone_index,                     // 骨骼索引 (k)
    int current_frame,                  // 當前幀數
    const FileDataForVMD& file_data,    // 檔案資料
    std::ofstream* log_file = nullptr,  // 可選的日誌檔案
    const std::string& debug_prefix = ""// 日誌前綴，用於區分不同的呼叫
)
{
    ...
    if (file_data.bone_name_map.find(bone_index) == file_data.bone_name_map.end() ||
        ...
        // 如果驗證失敗，返回預設的 bone_frame
        if (log_file && log_file->is_open()) {
            *log_file << debug_prefix << "WARNING: Bone name mismatch for index " << bone_index << "\n";
        }
        return bone_frame;
    }
    ...
    if (parent_index != -1 && file_data.parent_index_map.find(parent_index) != file_data.parent_index_map.end()) {
        parent_world = to_ummat(ExpGetPmdBoneWorldMat(model_index, parent_index));
        local = world * parent_world.inverted();
    }

    // 獲取父骨骼初始位置
    UMVec3f parent_initial_trans;
    parent_initial_trans[0] = parent_initial_trans[1] = parent_initial_trans[2] = 0.0f;
    ...
    // 日誌記錄
    if (log_file && log_file->is_open()) {
        *log_file << debug_prefix << "CALC_BONE_FRAME: Index=" << bone_index << ", Name=" << bone_name << "\n";
        *log_file << debug_prefix << "INIT_POS:" << initial_trans[0] << "," << initial_trans[1] << "," << initial_trans[2] << "\n";
        *log_file << debug_prefix << "PARENT_INIT_POS:" << parent_initial_trans[0] << "," << parent_initial_trans[1] << "," << parent_initial_trans[2] << "\n";
        *log_file << debug_prefix << "LOCAL_TRANS:" << local[3][0] << "," << local[3][1] << "," << local[3][2] << "\n";
    }
    ...
    if (log_file && log_file->is_open()) {
        *log_file << debug_prefix << "VMD_POS:" << bone_frame.position[0] << "," << bone_frame.position[1] << "," << bone_frame.position[2] << "\n";
        *log_file << debug_prefix << "VMD_ROT:" << bone_frame.orientation[0] << "," << bone_frame.orientation[1] << "," << bone_frame.orientation[2] << "," << bone_frame.orientation[3] << "\n";
    }
...
static bool execute_vmd_export(const int currentframe)
{
	...
	for (int i = 0; i < pmd_num; ++i)
	{
		...
		for (int k = 0; k < bone_num; ++k)
		{
			...
			// =================================================================
			if (log_file.is_open()) {
				log_file << "====== DEBUGGING FRAME:" << currentframe << ",BONE:" << k << ",NAME:" << bone_name << " ======\n";

				log_file << "DEBUG: is_ik_effector_bone? " << (is_ik_effector_bone ? "Yes" : "No") << "\n";
				log_file << "DEBUG: is_affected_by_ik? " << (is_affected_by_ik ? "Yes" : "No") << "\n";
				log_file << "DEBUG: is_fuyo_effector_bone? " << (is_fuyo_effector_bone ? "Yes" : "No") << "\n";
				log_file << "DEBUG: is_affected_by_fuyo? " << (is_affected_by_fuyo ? "Yes" : "No") << "\n";
				log_file << "DEBUG: is_simulated_physics_bone? " << (is_simulated_physics_bone ? "Yes" : "No") << "\n";
				log_file << "DEBUG: is_non_simulated_physics_bone? " << (is_non_simulated_physics_bone ? "Yes" : "No") << "\n";

				// 付与(Grant/Fuyo)資訊
				if (file_data.pmx && k < static_cast<int>(file_data.pmx->bones.size())) {
					const pmx::PmxBone& bone = file_data.pmx->bones[k];
					const uint16_t grant_flags = bone.bone_flag & 0x0300; // 0x0100 | 0x0200

					if (grant_flags) {
						log_file << "FUYO_INFO: Grant Child, Parent=" << bone.grant_parent_index
								<< ", Weight=" << bone.grant_weight
								<< ", Type=" << (grant_flags == 0x0100 ? "Rotation" :
												grant_flags == 0x0200 ? "Position" : "Both") << "\n";
					}

					if (is_fuyo_effector_bone) {
						log_file << "FUYO_INFO: Grant Parent\n";
					}
				}
			}
			// =================================================================
			...
			// 使用輔助函數計算 bone frame
			vmd::VmdBoneFrame bone_frame = calculate_bone_frame(i, k, currentframe, file_data, &log_file, "CURRENT_");

			// 如果這是付与子骨骼，則移除付与親的影響
			if (file_data.pmx && k < static_cast<int>(file_data.pmx->bones.size())) {
				const pmx::PmxBone& current_bone = file_data.pmx->bones[k];
				const uint16_t grant_flags = current_bone.bone_flag & 0x0300;

				if (grant_flags && current_bone.grant_parent_index >= 0 &&
					current_bone.grant_parent_index < static_cast<int>(file_data.pmx->bones.size())) {

					if (log_file.is_open()) {
						log_file << "GRANT_REMOVAL: Starting removal process\n";
						log_file << "GRANT_REMOVAL: Before - Pos[" << bone_frame.position[0] << "," << bone_frame.position[1] << "," << bone_frame.position[2] << "]\n";
						log_file << "GRANT_REMOVAL: Before - Rot[" << bone_frame.orientation[0] << "," << bone_frame.orientation[1] << "," << bone_frame.orientation[2] << "," << bone_frame.orientation[3] << "]\n";
					}

					// 計算付与親骨骼的 frame
					vmd::VmdBoneFrame grant_parent_frame = calculate_bone_frame(
						i, current_bone.grant_parent_index, currentframe, file_data, &log_file, "GRANT_PARENT_"
					);

					const float grant_weight = current_bone.grant_weight;

					// 移除位置付与影響
					if (grant_flags & 0x0200) { // 移動付与
					...
					// 移除旋轉付与影響 - 使用 Imath::Quat
					if (grant_flags & 0x0100) { // 旋轉付与
						// 建立 Imath 四元數物件
						...
						if (log_file.is_open()) {
							log_file << "GRANT_QUAT: Current quat (w,x,y,z) = (" << current_quat.r << "," << current_quat.v.x << "," << current_quat.v.y << "," << current_quat.v.z << ")\n";
							log_file << "GRANT_QUAT: Parent quat (w,x,y,z) = (" << parent_quat.r << "," << parent_quat.v.x << "," << parent_quat.v.y << "," << parent_quat.v.z << ")\n";
						}

						// 建立單位四元數用於插值
						Imath::Quatf identity = Imath::Quatf::identity();

						// 使用 slerp 計算付与親四元數按權重縮放後的結果
						...
						if (log_file.is_open()) {
							log_file << "GRANT_QUAT: Scaled parent quat (w,x,y,z) = (" << scaled_parent_quat.r << "," << scaled_parent_quat.v.x << "," << scaled_parent_quat.v.y << "," << scaled_parent_quat.v.z << ")\n";
						}

						// 移除付与影響：current_pure = current * scaled_parent_quat^(-1)
						...
						if (log_file.is_open()) {
							log_file << "GRANT_QUAT: Pure quat (w,x,y,z) = (" << pure_quat.r << "," << pure_quat.v.x << "," << pure_quat.v.y << "," << pure_quat.v.z << ")\n";
						}
					}

					if (log_file.is_open()) {
						log_file << "GRANT_REMOVAL: After - Pos[" << bone_frame.position[0] << "," << bone_frame.position[1] << "," << bone_frame.position[2] << "]\n";
						log_file << "GRANT_REMOVAL: After - Rot[" << bone_frame.orientation[0] << "," << bone_frame.orientation[1] << "," << bone_frame.orientation[2] << "," << bone_frame.orientation[3] << "]\n";
					}
				}
			}
