...
def export_rib(ribpath, framenumber):
    with open(ribpath, "a", encoding="utf-8") as ribfile:
        eye = get_camera_eye()
        up = get_camera_up()
        ...
        for buf in range(get_vertex_buffer_size()):
            ...
            for mat in range(get_material_size(buf)):
                ...
                ribfile.write(
                    ...
                    + alembic_dll_path
                    + '"'
                    + ' " -filename alembic_file.abc -fps 30 -frame '
                    + str(framenumber)
                    + " -flipv -objectpath /"
                    + object_name
                    + '" ] '
...
if framenumber == start_frame:
    ...
    start_alembic_export("", export_mode, export_scale, is_export_normals, is_export_uvs, is_use_euler_rotation_for_camera, is_use_ogawa)
